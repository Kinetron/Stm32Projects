...
char ssd1306_PrintChar(char ch, uint8_t scale) {
    ...
    for (uint8_t col = 0; col < 6; col++) 
    {
		...
		for(i = 0; i < 8 * scale; i++)
        {
            if((bits >> i) & 0x01) 
            {
                ssd1306_DrawPixel(SSD1306.CurrentX + col, (SSD1306.CurrentY + i), White);
            }
            else
            {
                ssd1306_DrawPixel(SSD1306.CurrentX + col, (SSD1306.CurrentY + i), Black);
            } 
        }
    }

     SSD1306.CurrentX += 5; 
    _spacesBetweenLetters(2, scale);
...
{
    for (uint8_t col = 0; col < lengthPx; col++) 
    {
